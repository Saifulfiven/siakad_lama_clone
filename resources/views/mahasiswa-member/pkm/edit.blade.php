        ...
        <div class="col-md-12">
          <section class="panel">
            ...
            <div class="panel-body" style="padding-top: 3px">
                ...
                <form action="{{ route('m_pkm_update') }}" id="form-pkm" method="post" data-collabel="3" data-alignlabel="left">
                    {{ csrf_field() }}
                    <input type="hidden" name="id" value="{{ $pkm->id }}">

                    <div class="row" style="border-bottom: 1px solid #eee;margin-bottom: 13px">
                        <div class="col-md-12">
                            <a href="{{ route('m_pkm') }}" onclick="window.history.back();" style="margin: 3px 3px" class="btn btn-success btn-sm pull-right"><i class="fa fa-times"></i> BATAL / KEMBALI</a>
                        </div>
                    </div>
                                    ...
                                    @foreach( $peserta as $val )
                                        <tr>
                                            ...
                                            <td align="center">
                                                ...
                                                <a href="{{ route('m_pkm_anggota_delete', ['id' => $val->id]) }}" onclick="return confirm('Anda ingin menghapus anggota ini?')" class="btn btn-danger btn-xs"><i class="fa fa-times"></i></a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                                ...
                                <table class="table table-bordered" style="min-width: 400px;width: 400px">
                                    ...
                                    @foreach( $pembimbing as $val )
                                        <tr>
                                            ...
                                            <td>{{ $val->nm_dosen }}</td>
                                            <td align="center">
                                                <a href="{{ route('m_pkm_dosen_delete', ['id' => $val->id]) }}" onclick="return confirm('Anda ingin menghapus pembimbing ini?')" class="btn btn-danger btn-xs"><i class="fa fa-times"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
...
<div id="modal-add-anggota" class="modal fade" tabindex="-1" style="top: 40% !important">
    ...
    <div class="modal-body">
        ...
        <div class="col-md-12">
            <form action="{{ route('m_pkm_anggota_store') }}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="id" value="{{ $pkm->id }}">
...
<div id="modal-add-dosen" class="modal fade" tabindex="-1" style="top: 40% !important">
    ...
    <div class="modal-body">
        ...
        <div class="col-md-12">
            <form action="{{ route('m_pkm_dosen_store') }}" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="id" value="{{ $pkm->id }}">
...
<script>
    $(function () {
        ...
        $('#autocomplete-ajax').autocomplete({
            serviceUrl: '{{ route('pkm_get_mahasiswa') }}',
        ...
        $('#autocomplete-ajax-dsn').autocomplete({
            serviceUrl: '{{ route('pkm_get_dosen') }}',
            lookupFilter: function(suggestion, originalQuery, queryLowerCase) {
                var re = new RegExp('\\b' + $.Autocomplete.utils.escapeRegExChars(queryLowerCase), 'gi');
                return re.test(suggestion.value);
            },
        ...
        var options = {
            ...
            success:function(data, status, message) {
                if ( data.error == 1 ) {
                    ...
                    window.location.href='{{ route('m_pkm') }}';
                }
            },
