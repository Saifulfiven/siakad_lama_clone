...
class KrsController extends Controller
{
    public function index(Request $r)
    {
        ...
        if ($mhs->id_prodi == 61101) {
            ...
            return $this->indexS2($mhs, $periode, $r);
    ...
    private function indexS2($mhs, $periode, $r)
    {
        $krs_stat = $this->cekStatusKrs($mhs);
        $posisi_semester = $this->getSemester($mhs->id);
        ...
        if ($krs_stat['status'] == 'lock') {
            ...
            if ($krs_stat['jenis'] == 1) {
                ...
                if ($mhs->jenis_daftar == 1) {
                    ...
                    if ($posisi_semester <= 4) {

                        $di_krs_tmp = $this->diKrsTmp($mhs->id);
                        $telah_diprogram = $this->telahDiprogram($mhs->id);
                        $konsentrasi = DB::table('konsentrasi_pps')
                            ->where('id_mhs_reg', $mhs->id)
                            ->orderBy('created_at', 'desc')
                            ->get();
                        // dd($konsentrasi);

                        if (!empty($konsentrasi->id_konsentrasi)) {
                            $data['matakuliah'] = $this->getMkPdb($mhs, $posisi_semester, $telah_diprogram, $di_krs_tmp)
                                ->union($this->mkKonsentrasiS2($mhs, $posisi_semester, $telah_diprogram, $di_krs_tmp, $konsentrasi->id_konsentrasi))
                                ->orderBy('smt')
                                ->orderBy('jenis_mk', 'asc')
                                ->groupBy('id_mk')
                                ->get();
                        ...
                        $data['mk_error'] = $this->mkErrorS2($mhs)->get();
        ...
        return view('mahasiswa-member.krs.indexS2', $data);

    }

    private function getSemester($id_mhs_reg)
    {
        $transkrip = DB::select("SELECT mkur.smt
                                FROM nilai as nil
                                ...
                                order by mkur.smt desc limit 1");

        if (empty($transkrip)) {
            $smt = 1;
        } else {
            $smt = $transkrip[0]->smt + 1;
        }
    ...
    private function getMkPdb($mhs, $smstr, $telah_diprogram, $di_krs_tmp, $jenis = 1)
    {
        ...
        $data = DB::table('kurikulum as kur')
            ...
            ->where(function ($q) {
                $q->where('mk.id_konsentrasi', 0)
                    ->orWhere('mk.id_konsentrasi', null);
            })
            ->orderBy('mk.jenis_mk');
    ...
    private function mkKonsentrasiS2($mhs, $smstr, $telah_diprogram, $di_krs_tmp, $konsentrasi)
    {
        ...
        $data = DB::table('kurikulum as kur')
            ...
            ->whereNotIn('mkur.id_mk', $di_krs_tmp)
            ->whereNotIn('mkur.id_mk', $telah_diprogram)
            ->where('mk.id_konsentrasi', $konsentrasi);
        // dd($data->get());
        return $data;
    }
    ...
    private function mkError($mhs, $jenis = 1)
    {
        ...
        $data = DB::table('kurikulum as kur')
            ...
            ->select('mkur.id as id_mkur', 'mk.id as id_mk', 'mk.jenis_mk', 'mk.mk_terganti', 'mk.kode_mk', 'mk.nm_mk', 'mk.sks_mk', 'mkur.smt', DB::raw('\'error\' as asal'))
    ...
    private function mkErrorS2($mhs, $jenis = 1)
    {
        $pengganti = $this->mkPengganti($mhs->id);
        $telah_diprogram = $this->telahDiprogram($mhs->id);
    ...
    private function mkTransfer($mhs, $jenis = 1)
    {
        $data = DB::table('jadwal_kuliah as jdk')
            ...
            ->leftJoin('matakuliah as mk', 'mk.id', 'mkur.id_mk')
            ->select('mkur.id as id_mkur', 'mkur.id_mk', 'mk.jenis_mk', 'mk.mk_terganti', 'mk.kode_mk', 'mk.nm_mk', 'mk.sks_mk', 'mkur.smt', DB::raw('\'transfer\' as asal'))
        // ->select('mkur.id as id_mkur', 'mk.id as id_mk','mk.jenis_mk','mk.mk_terganti','mk.kode_mk','mk.nm_mk','mk.sks_mk', 'mkur.smt', DB::raw('\'error\' as asal'))
        // ->where('mkur.id_kurikulum', $mhs->id_kurikulum)
    ...
    public function storeTmp(Request $r)
    {
        try {
            DB::transaction(function () use ($r) {
                ...
                $max_sks = $prodi == 61101 ? 18 : 24;

                if ($r->sks + $krs_diambil > $max_sks) {
                    return Response::json(['error' => 1, 'msg' => 'Jumlah SKS yang diprogram tidak boleh melebihi ' . $max_sks . ' SKS']);
                }
    ...
    public function storeTmpArr(Request $r)
    {
        try {
            ...
            foreach ($data_krs as $val) {
                ...
                if ($val->sks + $sks_diambil > $max_sks) {
                    Rmt::error('Matakuliah tidak tersimpan semua. Jumlah SKS yang diprogram tidak boleh melebihi ' . $max_sks . ' SKS');
                    return Response::json(['error' => 1, 'msg' => $r->sks . '-' . $sks_diambil . 'Jumlah SKS yang diprogram tidak boleh melebihi ' . $max_sks . ' SKS']);
                    break;
    ...
    public function store(Request $r)
    {
        try {
            ...
            if ($sks_diambil > $max_sks) {
                Rmt::error('Jumlah SKS yang diprogram tidak boleh melebihi ' . $max_sks . ' SKS. Silahkan hapus beberapa matakuliah');
    ...
    // private function storeToJadwal($data)
    // {
    //     $data = DB::table('krs_mhs as km')
    //             ->leftJoin('mahasiswa_reg as m', 'km.id_mhs_reg', 'm.id')
    //             ->select('km.*', 'm.nim')
    //             ->where('km.id_mhs_reg', $data['id_mhs_reg'])
    //             ->where('km.id_smt', $data['id_smt'])
    //             ->get();
