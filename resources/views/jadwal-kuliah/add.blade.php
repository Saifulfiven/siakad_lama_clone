...
@section('content')
                    ...
                    <div class="row">
                        <div class="col-md-7">
                            ...
                            <div class="data-jadwal" style="display: none">
                                ...
                                <div class="form-group" id="show-prodi">
                                ...
                                <div class="form-group info-peserta" style="display: none">
                                    <label class="control-label">&nbsp;</label>
                                    <div id="info-peserta"></div>
                                </div>
    ...
    <div id="modal-msg" class="modal fade" tabindex="-1" style="top:30%">
        ...
        <div class="modal-body">
            ...
            <hr>
            <div class="alert alert-warning data-msg2" style="display: none"></div>
            <center>
                <a href="{{ route('jdk') }}" class="btn btn-sm btn-info">Kembali</a>
                <a href="{{ route('jdk_add') }}" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Tambah Jadwal</a>
            </center>
        </div>
        <!-- //modal-body-->
    </div>
...
<script>
    $(function () {
        ...
        $('#prodi').change(function(){
            ...
            $.ajax({
                url: '{{ route('jdk_ajax') }}',
                data: {tipe: 'kelas_s1', prodi: prodi },
            ...
            var options = {
                ...
                success:function(data, status, message) {
                    ...
                    if ( data.error == 1 ) {
                        ...
                        let respon = parseObj(data.peserta);
                        ...
                        if ( respon.length == 0 ) {
                            ...
                            for ( let i = 0; i < 2; i++ ){
                                ...
                                if ( respon[i] == 0 ) continue;

                                pesan += "- "+respon[i]+"<br>";
                            }

                            let pesan2 = '';
                            for ( let i = 0; i < respon[2].length; i++ ){
                                pesan2 += "- "+respon[2][i]+"<br>";
                            }
    ...
    function kelasChange(value)
    {
        let id_mkur = $('#id-mkur').val();

        $.ajax({
            url: '{{ route('jdk_ajax') }}',
            data: {tipe: 'cek-kelas', id_mkur: id_mkur, kelas:value},
            beforeSend: function( xhr ) {
                $('.info-peserta').show();
                $('#info-peserta').html('<i class="fa fa-spinner fa-spin"></i> mengecek peserta...');
            },
            success: function(data){
                $('#info-peserta').html('<b>'+data+'</b>');
            },
            error: function(data,status,msg){
                alert('Terjadi gangguan saat mengecek jumlah peserta');
            }
        });
    }
