...
<div class="kontainer">
    ...
    <img src="{{ url('resources/assets/img/kop.jpg') }}" width="100%" style="margin-top: -2px;">
    <hr>
    <center>
    ...
    pada tanggal {{ Rmt::tgl_indo($skripsi->tgl_ujian) }}, pada pukul {{ $skripsi->pukul }} WITA dengan hasil sebagai berikut:
    <br>
    ...
    <table border="1" width="100%">
        ...
        <tbody align="center">
            ...
            <?php 
                $nilai = DB::table('nilai_seminar')
                        ->where('id_penguji', $p->id)
                        ->orderBy('kriteria_penilaian', 'asc')
                        ->get()->toArray();
            ?>

                @foreach( Sia::kriteriaPenilaian($mhs->id_prodi) as $key => $val )
                    
                    <?php $nil = array_key_exists($key, $nilai) ? $nilai[$key]->nilai : 0 ?>
    ...
    <table border="0">
        <tr>
            ...
            <td rowspan="2">=</td>
            <td rowspan="2">{{ $total/$pembagi }}</td>
            <td rowspan="2">= &nbsp; <b>{{ Sia::grade($mhs->id_prodi, $total/$pembagi) }}</b></td>
        </tr>
        <tr>
    ...
    <table width="100%">
        ...
        <tr>
            ...
            <td align="center" class="footer-ttd">
                @if ( !empty($p->ttd) )
                    <img src="{{ Rmt::linkTtd($p->ttd) }}" width="150">
                @else
                    <br><br><br><br><br>
                @endif
            </td>
