...
<div class="col-md-12">
	<center>
	<h4>JADWAL UJIAN ({{ Session::get('jdu_jenis_ujian') }})<br>
	...
	<table border="0">
		<tr>
			...
			<td>: {{ Session::get('jdu_semester') }}</td>
		</tr>
		<tr>
			<td>Program Studi</td>
			<td>: {{ Session::get('jdu_prodi') }}</td>
		</tr>
		<tr>
			<td>Jenis Ujian</td>
	...
	<table border="1" class="table" width="100%" id="tbl">
		...
		<tbody>
			@foreach( $jadwal as $r )
				<tr>
					<td align="center">{{ Carbon::parse($r->tgl_ujian)->format('d-m-Y') }}</td>
					<td align="center">{{ Rmt::hari($r->hari) }}</td>
					<td align="center">{{ substr($r->jam_masuk,0,5) }} - {{ substr($r->jam_selesai,0,5) }}</td>
