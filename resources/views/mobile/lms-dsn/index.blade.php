...
<div id="content" style="padding: 20px 5px 5px 5px">

    <div class="row" >
        <div class="col-md-12" style="">
            <div class="col-md-12" style="padding: 0">
                <button class="btn btn-theme btn-xs" data-toggle="modal" data-target="#modal-panduan">Panduan</button> &nbsp;
                <button class="btn btn-primary btn-xs" data-toggle="modal" data-target="#modal-undang">Masukkan Peserta</button>

                @if ( count($peserta_undangan) > 0 )
                    <span class="tooltip-area">
                        &nbsp; <button class="btn btn-primary btn-transparent btn-xs" data-toggle="modal" title="Peserta Undangan" data-target="#md-peserta-undangan"><i class="fa fa-users"></i></button>
                    </span>
                @endif
                ...
                @if ( count($undangan) > 0 )
                    <span class="tooltip-area">
                        &nbsp;
                        <button class="btn btn-default btn-transparent btn-xs notif-persetujuan" style="color: #f35958" data-toggle="modal" data-target="#md-notification" title="Permintaan Baru">
            ...
            <div class="input-group col-md-12" style="padding-left: 0 !important; padding-right: 0 !important; max-width: 400px">
                <select class="form-control input-sm" id="pertemuan-ke" style="height: 22px;padding-top: 2px;font-size: 11px">
                ...
                <span class="input-group-btn">
                    <button class="btn btn-theme-inverse btn-xs" type="button" onclick="openDokumen()"><i class="fa fa-plus"></i> Tambah dari Dokumen Saya</button>
                </span>
            ...
            <div class="col-md-12" style="padding: 0">
                <button class="btn btn-default toggle-forum btn-xs"><i class="fa fa-comments"></i> Forum <i class="karet fa fa-caret-down"></i></button>
            </div>
        </div>
        ...
        <div class="col-md-12 learning">
            ...
            <div class="forum" style="display: none">
                ...
                <div class="forum-footer">
                    <button class="toggle-forum btn btn-theme btn-xs"><i class="fa fa-times"></i> Tutup</button>
                    <button class="btn btn-theme-inverse btn-xs" data-toggle="modal" data-target="#modal-add-topik"><i class="fa fa-comments-o"></i> Buat Topik</button>
                </div>
            </div>
                ...
                <section class="panel">
                    <header class="panel-heading">
                        ...
                        <div class="btn-add pull-right">
                            <button class="btn btn-primary btn-xs btn-add-res" data-id="{{ $r->id }}" data-prt="{{ $i }}"><i class="fa fa-plus"></i> Tambah</button>
                            <!-- <div class="btn-group btn-loading">
                                <button type="button" class="btn btn-primary btn-xs dropdown-toggle" data-toggle="dropdown"> <i class="fa fa-plus"></i> Tambah <span class="caret"></span> </button>
                                <ul class="dropdown-menu custom-dropdown align-xs-left" role="menu" style="background-color: #0075b0">
                                    <li><a href="{{ route('dsnm_lms_add_materi', ['id' => $r->id, 'prt' => $i, 'id_dosen' => $id_dosen]) }}">Materi</a></li>
                                    <li><a href="{{ route('dsnm_lms_tugas_add', ['id' => $r->id, 'prt' => $i, 'id_dosen' => $id_dosen]) }}">Tugas</a></li>
                                    <li><a href="{{ route('m_kuis_add', ['id' => $r->id, 'prt' => $i, 'id_dosen' => $id_dosen]) }}">Kuis</a></li>
                                    <li><a href="{{ route('m_video_add', ['id' => $r->id, 'prt' => $i, 'id_dosen' => $id_dosen]) }}">Video</a></li>
                                    <li><a href="{{ route('dsnm_lms_catatan_add', ['id' => $r->id, 'prt' => $i, 'id_dosen' => $id_dosen]) }}">Catatan</a></li>
                                </ul>
                            </div> -->
                        </div>
                    </header>
                    ...
                    <div class="panel-body" style="padding: 3px 3px;">
                        <?php $data = Rmt::getResources($i, $r->id, $id_dosen); ?>
                            ...
                            <div id="sortable-<?= $i ?>">
                                ...
                                @foreach( $data as $res )
                                    ...
                                    <div class="col-md-12 container-resources" id="urut-<?= $res->id ?>">
                                        ...
                                        @if ( $res->jenis == 'materi' )
                                            <a href="{{ route('dsnm_lms_materi_view', ['id_materi' => $res->id_resource, 'id_dosen' => $id_dosen, 'file' => $res->file]) }}" target="_blank">
                                        @elseif( $res->jenis == 'tugas' )
                                            <a href="{{ route('dsnm_lms_tugas_detail', ['id_jadwal' => $r->id, 'id' => $res->id_resource, 'id_dosen' => $id_dosen]) }}" class="btn-loading">
                                        @elseif ( $res->jenis == 'kuis' )
                                            <a href="{{ route('m_kuis_detail', ['id_jadwal' => $r->id, 'id' => $res->id_resource, 'id_dosen' => $id_dosen]) }}">
                                        @elseif ( $res->jenis == 'video' )
                                            <a href="{{ route('m_video_detail', ['id' => $r->id, 'id_video' => $res->id_resource, 'id_dosen' => $id_dosen]) }}">
                                        @else
                                            <a class="catatan">
                                        @endif
                                        ...
                                        <span class="btn-aksi pull-right btn-loading">
                                            <?php
                                                if ( $res->jenis == 'materi' ) {
                                                    $link_edit = route('dsnm_lms_materi_edit', ['id' => $r->id]).'?id_materi='.$res->id_resource;
                                                } elseif ( $res->jenis == 'tugas' ) {
                                                    $link_edit = route('dsnm_lms_tugas_edit', ['id' => $r->id]).'?id_tugas='.$res->id_resource;
                                                } elseif ( $res->jenis == 'kuis' ) {
                                                    $link_edit = route('m_kuis_edit', ['id' => $r->id, 'id_kuis' => $res->id_resource]);
                                                } elseif ( $res->jenis == 'catatan' ) {
                                                    $link_edit = route('dsnm_lms_catatan_edit', ['id' => $r->id]). '?id_catatan='.$res->id_resource;
                                                }  elseif ( $res->jenis == 'video' ) {
                                                    $link_edit = route('m_video_edit', ['id' => $r->id, 'id_video' => $res->id_resource]);
                                                }
                                            ?>
                                            <a href="{{ $link_edit }}&id_dosen={{ $id_dosen }}" class="btn btn-default btn-xs">Ubah</a>
                                            <a href="{{ route('dsnm_lms_delete_resources') }}?id={{ $res->id }}&jenis={{ $res->jenis }}&id_dosen={{ $id_dosen }}" onclick="return confirm('Anda ingin menghapus data ini?')" class="btn btn-danger btn-xs">Hapus</a>
                                        </span>
                                    </div>
                        ...
                        @else
                            <div id="sortable-<?= $i ?>" style="margin-left: 10px">
...
<div id="modal-panduan" class="modal fade" data-width="700" tabindex="-1">
    ...
    <div class="modal-body" style="padding-top: 0">
        <iframe frameborder="0" style="position: relative; top:20%;left:0;width:100%;height: 350px; border: none" src="https://www.youtube.com/embed/YpVGg7ZPOQM" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
</div>
...
<div id="modal-add-topik" class="modal fade" data-width="700" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body" style="padding-top: 0">
        <div class="row">
            <div class="col-lg-12">
                <form action="{{ route('dsnm_lms_topik_store') }}" method="post" id="form-add-topik">
                    {{ csrf_field() }}
...
<div id="modal-edit-topik" class="modal fade" data-width="700" tabindex="-1">
    ...
    <div class="modal-body" style="padding-top: 0;height: 340px">
        <div class="row">
            <div class="col-lg-12">
                <form action="{{ route('dsnm_lms_topik_update') }}" method="post" id="form-edit-topik">
                ...
                <tbody>
                    @foreach( $peserta_undangan as $pu )
                        <tr>
                            ...
                            <td>
                                <span class="tooltip-area">
                                <a href="{{ route('dsnm_lms_approval_mhs', [$id_jadwal, $pu->id_peserta]) }}?approv=0"
                                    title="Keluarkan"
                ...
                <li>
                    ...
                    <div class="im-confirm-group">
                        <div class=" btn-group btn-group-justified">
                            <a class="btn btn-inverse" href="{{ route('dsnm_lms_approval_mhs', [$id_jadwal, $ud->id_peserta]) }}?approv=1" onclick="return confirm('Anda akan memasukkan mahasiswa ini dalam kelas anda?')">Setujui</a>
                            <a class="btn btn-theme im-confirm" href="{{ route('dsnm_lms_approval_mhs', [$id_jadwal, $ud->id_peserta]) }}?approv=0" onclick="return confirm('Anda menolak mahasiswa ini?')">Tolak</a>
                        </div>
                    </div>
                </li>
                @endforeach
...
<div id="modal-add" class="modal fade container">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
        <h4 class="modal-title">Tambah Sumber Daya</h4>
    </div>
    <!-- //modal-header-->
    <div class="modal-body">
        <a id="add-materi" class="btn btn-primary btn-md btn-loading btn-block" href="">M A T E R I</a>
        <a id="add-tugas" class="btn btn-primary btn-md btn-loading btn-block" href="">T U G A S</a>
        <a id="add-kuis" class="btn btn-primary btn-md btn-loading btn-block" href="">K U I S</a>
        <a id="add-video" class="btn btn-primary btn-md btn-loading btn-block" href="">V I D E O</a>
        <a id="add-catatan" class="btn btn-primary btn-md btn-loading btn-block" href="">C A T A T A N</a>
    </div>
    <div class="modal-footer">
        <button type="button" data-dismiss="modal" class="btn btn-md btn-danger btn-block">B A T A L K A N</button>
    </div>
    <!-- //modal-body-->
</div>
@endsection
...
<script>
    ...
    $(function(){
        ...
        $('.btn-add-res').click(function(){
            var div = $(this);
            var id = div.data('id');
            var prt = div.data('prt');
            var add_materi = '{{ route('dsnm_lms_add_materi') }}/'+id+'?prt='+prt+'&id_dosen={{ $id_dosen }}';
            var add_tugas = '{{ route('dsnm_lms_tugas_add') }}/'+id+'?prt='+prt+'&id_dosen={{ $id_dosen }}';
            var add_kuis = '{{ route('m_kuis_add') }}/'+id+'?prt='+prt+'&id_dosen={{ $id_dosen }}';
            var add_video = '{{ route('m_video_add') }}/'+id+'?prt='+prt+'&id_dosen={{ $id_dosen }}';
            var add_catatan = '{{ route('dsnm_lms_catatan_add') }}/'+id+'?prt='+prt+'&id_dosen={{ $id_dosen }}';
            $('#add-materi').attr('href', add_materi);
            $('#add-tugas').attr('href', add_tugas);
            $('#add-kuis').attr('href', add_kuis);
            $('#add-video').attr('href', add_video);
            $('#add-catatan').attr('href', add_catatan);
    ...
    function appendData(indeks, file, id, id_materi)
    {
        ...
        var html = '<div class="col-md-12 container-resources" id="urut-'+id+'">'
                        +'<label class="container-cek">'
                            ...
                            +'<span class="checkmark"></span>'
                        +'</label>'
                        +'<a href="{{ route('dsnm_lms_materi_view') }}/'+id_materi+'/{{ $id_dosen }}/'+file+'" target="_blank">'
                            +'<div class="icon-resources">'
                                +'<img width="24" src="{{ url('resources') }}/assets/img/icon/"'+gambar+' />'
                            +'</div>'+judul[0]+'</a>'
                        +'<div class="pull-right">'
                        +'<a href="{{ route('dsnm_lms_materi_edit', ['id' => $r->id]) }}?id_materi='+id_materi+'" class="btn btn-default btn-xs">Ubah</a> '
                        +'<a href="{{ route('dsnm_lms_delete_resources') }}?id='+id+'&jenis=materi" onclick="return confirm(\'Anda ingin menghapus data ini?\')" class="btn btn-danger btn-xs">Hapus</a>'
                        +'</div>'
                    +'</div>';
    ...
    function updateUrutan(data)
    {
        ...
        $.ajax({
            url: '{{ route('dsnm_lms_update_urutan') }}',
            type: 'POST',
    ...
    function pilih(id, id_jadwal)
    {
        ...
        $.ajax({
            url: '{{ route('dsnm_lms_upload_materi') }}',
            type: 'POST',
            headers: { 'X-CSRF-TOKEN': token },
            data : {
                preventCache : new Date(),
    ...
    function pindahkan(indeks, tujuan)
    {
        ...
        $.ajax({
            url: '{{ route('dsnm_lms_pindah_pertemuan') }}',
            data : {
    ...
    function ubahTopik(id, judul, konten){
        ...
        $.get('{{ route('dsnm_lms_topik_edit') }}/'+id, function(data){
    ...
    function gabung(id)
    {
        if ( confirm('Anda ingin memasukkan mahasiswa ini?') ) {
            window.location.href="{{ route('dsnm_lms_undang_mhs') }}/{{ $id_jadwal }}/"+id;
        }
    }
