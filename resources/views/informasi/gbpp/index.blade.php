...
@section('contents')
	...
	<div class="content">
		<div class="row title-page">
			<div class="col-md-12">
				<h1>RPS &nbsp;<small>Daftar RPS</small></h1>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-md-12">
				...
				<div class="widget animated fadeInLeft delay-1">
					<div class="widget-head" style="padding-bottom: 0">
						<h2 class="widget-title">Prodi &nbsp;
							<select onchange="window.location.href=this.value">
								<?php $prodi = ['61201' => 'Manajemen','62201' => 'Akuntansi'] ?>
								<option value="{{ route('gbpp') }}">Semua</option>
								@foreach( Sia::listProdi() as $pr )
									<option value="{{ route('gbpp', ['prodi' => $pr->id_prodi]) }}" {{ $pr->id_prodi ==  Request::get('prodi') ? 'selected':'' }}>{{ $pr->jenjang .' '.$pr->nm_prodi }}</option>
								@endforeach
							</select>
							<br>
							<br>
					...
					<div class="widget-option" style="margin-top: -40px;margin-right: 5px"><a href="{{ route('gbpp_create') }}" id="add-account">+ GBPP Baru</a></div>
								...
								@if ( $gbpp->total() > 0 )
									...
									<table class="table lists absen">
										<tr class="first">
											...
											<td colspan="2">Tandai Semua &nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="delete" onclick="DeleteMassal('check', '{{ route('gbpp_delete') }}')"><i class="fa fa-trash-o"></i>&nbsp; Hapus</a></td>
										</tr>
											...
											<tr>
												...
												<td class="text-right action">
													<div class="btn-group" role="group">
														<a href="{{ $r->link }}" target="blank" class="btn btn-default"><i class="fa fa-search-plus"></i></a>
														<a href="{{ route('gbpp_edit',['id' => $r->id]) }}" class="btn btn-default"><i class="fa fa-pencil"></i></a>
														<a href="javascript:;" class="btn btn-default" onclick="Delete('{{ route('gbpp_delete', ['id' => $r->id])}}')"><i class="fa fa-trash-o"></i></a>
													</div>
												</td>
											</tr>
										@endforeach
									...
									<div class="text-center">
										{{ $gbpp->appends(array_except(Request::query(), 'page'))->links() }}
									</div>
