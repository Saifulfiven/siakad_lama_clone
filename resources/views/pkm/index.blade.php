...
@section('content')
                        ...
                        <div class="col-md-12">
                            ...
                            <table border="0" width="100%" style="margin-bottom: 10px">
                                <tr>
                                        ...
                                        <select class="form-control input-sm" onchange="filter('smt', this.value)">
                                            ...
                                            @foreach (Sia::listSemester() as $sm)
                                                <option value="{{ $sm->id_smt }}"
                                                    {{ Session::get('pkm.smt') == $sm->id_smt ? 'selected' : '' }}>
                                                    {{ $sm->nm_smt }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td></td>
                                    <td width="300px">
                                        <form action="{{ route('pkm_cari') }}" method="post" id="form-cari">
                                            <div class="input-group pull-right">
                                                {{ csrf_field() }}
                                                <input type="text" class="form-control input-sm" name="cari"
                                                    value="{{ Session::get('pkm.cari') }}">
                                                <div class="input-group-btn">
                                                    <button class="btn btn-default btn-sm" id="reset-cari" type="button"><i
                                    ...
                                    <td width="110px">
                                        <a href="{{ route('pkm_daftar') }}" class="btn btn-sm btn-primary pull-right"><i
                                                ...
                                                <td>
                                                    <span class="tooltip-area">
                                                        <a href="javascript::void(0);" class="btn btn-primary btn-xs"
                                                            title="Lihat" onclick="detail('{{ $r->id }}')"><i
                                                                class="fa fa-search-plus"></i></a> &nbsp; &nbsp;

                                                        <a href="{{ route('pkm_edit', ['id' => $r->id]) }}"
                                                            class="btn btn-warning btn-xs" title="Ubah"><i
                                                                class="fa fa-pencil"></i></a>

                                                        @if (Sia::canAction($r->id_smt))
                                                            &nbsp; &nbsp;
                                                            <a href="{{ route('pkm_delete', ['id' => $r->id]) }}"
    ...
    <div id="modal-detail" class="modal fade container" data-width="800" style="top: 20% !important" tabindex="-1">
        ...
        <div class="modal-body">
            ...
            <div class="col-md-12" id="content-detail"></div>
            <div class="col-md-12">
                <hr>
...
@section('registerscript')
    ...
    <script>
        ...
        function filter(modul, value) {
            window.location.href = '{{ route('pkm_filter') }}?modul=' + modul + '&val=' + value;
        }

        function detail(id) {
            $('#modal-detail').modal('show');
            $.ajax({
                url: '{{ route('pkm_detail') }}/' + id,
                data: {
                ...
                error: function(data, status, msg) {
                    alert(
                        'Terjadi gangguan saat mengambil data, periksa koneksi internet dan muat ulang halaman');
                }
            });
        }
    </script>
