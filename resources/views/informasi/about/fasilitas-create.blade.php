@extends('informasi.layout.main')

@section('title', 'Tambah Fasilitas')

@section('contents')

	<div class="content">

		<div class="row">
			<div class="col-md-12">
				<div class="widget animated fadeInLeft delay-1">
					<div class="widget-head">
						<h2 class="widget-title text-uppercase">
							Fasilitas Baru
							<div class="pull-right">
								<a href="{{ route('fasilitas') }}" class="btn btn-primary btn-sm">Kembali</a>
							</div>
						</h2>
					</div>
					<div class="widget-body">

							{{ Rmt::AlertSuccess() }}
							{{ Rmt::AlertError() }}
							{{ Rmt::AlertErrors($errors) }}
							<form action="{{ route('fasilitas_store') }}" method="post" enctype="multipart/form-data" class="form-custom tab-content">
								{{ csrf_field() }}
								<input type="hidden" name="urutan" value="{{ Request::get('urutan') }}">
								<div class="row tab-pane active" role="tabpanel">
									<div class="form-group col-md-8 col-sm-12">
										<label for="">Nama Fasilitas</label>
										<input type="text" name="nama_fasilitas" value="{{ old('nama_fasilitas') }}" class="form-control">
									</div>
									<div class="form-group col-md-12">
										<label for="">Keterangan</label>
										<textarea name="keterangan" id="editor" rows="10" cols="80">
							                {{ old('keterangan') }}
							            </textarea>
									</div>

									<div class="form-group col-md-4 col-sm-12">
										<label for="">Gambar</label>
										<input type="file" name="gambar" accept="image/*" value="{{ old('gambar') }}" class="form-control">
									</div>

									<div class="col-md-12">
										<br>
										<button class="btn btn-info">Simpan</button>
									</div>
								</div>
							</form>
					</div>
				</div>
			</div>
		</div>
	</div>

@endsection

@section('registerscript')
	<script src="{{ url('resources') }}/assets/informasi/ckeditor/ckeditor.js"></script>
	<script>
		$('.menu-about').addClass('actived');
		$('.form-custom').on('submit', function(){
			$(this).find('button').attr('disabled','');
		});
		CKEDITOR.replace( 'editor' );

	</script>

@endsection
