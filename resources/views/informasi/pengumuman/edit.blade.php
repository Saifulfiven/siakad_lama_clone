			...
			<div class="col-md-12">
				<div class="widget animated fadeInLeft delay-1">
					<div class="widget-head">
						<h2 class="widget-title text-uppercase">Ubah Pengumuman</h2>
					</div>
					<div class="widget-body">
							...
							<form action="{{ route('pengumuman_update') }}" method="post" class="form-custom tab-content">
								{{ csrf_field() }}
									...
									<div class="form-group col-md-8 col-sm-12">
										...
										<select name="kategori" class="form-control" style="width: 150px">
											@foreach( Rmt::kategoriInformasi() as $k )
												<option value="{{ $k }}" {{ $pengumuman->kategori == $k ? 'selected':'' }}>{{ $k }}</option>
											@endforeach
										</select>
									</div>
