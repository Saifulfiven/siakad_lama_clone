						...
						<div class="table-responsive">
							<table border="0" width="100%" style="min-width: 550px;margin-bottom: 10px">
								<tr>
									...
									<td></td>
									<td width="250px">
										<form action="{{ route('dsn_bim_cari') }}" method="post" id="form-cari">
											<div class="input-group pull-right">
												{{ csrf_field() }}
												<input type="text" class="form-control" name="cari" value="{{ Session::get('bim.cari') }}">
									...
									<td width="120">
										<button onclick="openCetak()" class="btn btn-primary pull-right"><i class="fa fa-print"></i> Cetak SK</button>
									</td>
								</tr>
							</table>
							...
							<table cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-striped table-hover">
								...
								<tbody align="center">
									@foreach( $bimbingan as $bim )
										<tr>
											<td>{{ $loop->iteration - 1 + $bimbingan->firstItem() }}</td>
											<td align="left">{{ $bim->nim }}</td>
											<td align="left">{{ $bim->nm_mhs }}</td>
											<td>{{ $bim->jenjang }} - {{ $bim->nm_prodi }}</td>
											<td>
												<a href="{{ route('dsn_bim_detail', ['id_mhs_reg' => $bim->id_mhs_reg, 'id_smt' => $bim->id_smt]) }}" class="btn btn-primary btn-sm">Buka Bimbingan</a>
											</td>
										</tr>
									@endforeach
								</tbody>
							...
							@if ( $bimbingan->total() == 0 )
								&nbsp; Tidak ada data
							@endif

							@if ( $bimbingan->total() > 0 )
								<div class="pull-left">
									Jumlah data : {{ $bimbingan->total() }}
								</div>
							@endif
...
<div id="modal-sk" class="modal fade md-stickTop" tabindex="-1">
    ...
    <div class="modal-body">
        <form action="{{ route('dsn_sk_bimbingan') }}" target="_blank" method="post">
        	{{ csrf_field() }}
...
<script>
    ...
    {
        window.location.href = '{{ route('dsn_bim_filter') }}?modul='+modul+'&val='+value;
    }
    ...
    function openCetak()
    {
    	...
    	$.ajax({
    		url : '{{ route('dsn_sk_bimbingan_data') }}',
