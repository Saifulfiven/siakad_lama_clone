...
<div class="col-md-12">
	...
	<table border="0">
		<tr>
			...
			<td>: {{ is_array(Session::get('jdk_smt')) ? implode(',',Session::get('jdk_smt')) : 'Semua' }}</td>
		</tr>
		<tr>
			<td>Program Studi</td>
			<td>: 
				@if ( is_array(Session::get('jdk_prodi')) )
					@foreach( Session::get('jdk_prodi') as $val )
						<?php $prodi = Sia::dataProdi($val); ?>
						{{ $prodi->jenjang.' '.$prodi->nm_prodi }}
					@endforeach
				@else
					Semua
				@endif
		...
		<tr>
			...
			<td>: {{ is_array(Session::get('jdk_ket')) ? implode(',',Session::get('jdk_ket')) : 'Semua' }}</td>
		</tr>
	</table>
