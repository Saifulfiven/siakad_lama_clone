        ...
        <div class="col-md-12">
          <section class="panel">
            ...
            <div class="panel-body" style="padding-top: 3px">
                <form action="{{ route('jda_mhs_store_arr') }}" id="form-mahasiswa" method="post">
                    {{ csrf_field() }}
                    ...
                    <div class="row" style="margin-bottom: 13px">
                        <div class="col-md-12">
                            ...
                            <a href="{{ route('jda_detail', ['id'=> $r->id]) }}" style="margin: 3px 3px" class="btn btn-info btn-sm pull-right"><i class="fa fa-refresh"></i> BATAL</a>
                                ...
                                <table border="0" width="100%" style="margin-bottom: 10px">
                                    <tr>
                                        ...
                                        <td width="200px" style="padding-left: 3px">
                                            <select class="form-control input-sm" id="prodi">
                                                @foreach( Sia::listProdi() as $pr )
                                                    <option value="{{ $pr->id_prodi }}" {{ Request::get('pr') == $pr->id_prodi ? 'selected':'' }}>{{ $pr->jenjang }} - {{ $pr->nm_prodi }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td></td>
...
<script>
    ...
    $(function () {
        $('#angkatan').change(function(){
            ...
            window.location.href='{{ route('jda_mhs_add') }}?jdk={{ Request::get('jdk') }}&pr={{ Request::get('pr') }}&ang='+angkatan;
        })
        $('#prodi').change(function(){
            var prodi = $(this).val();
            window.location.href='{{ route('jda_mhs_add') }}?jdk={{ Request::get('jdk') }}&pr='+prodi+'&ang={{ Request::get('ang') }}';
        })
    });
        ...
        var options = {
            ...
            success:function(data, status, message) {
                if ( data.error == 1 ) {
                    ...
                    window.location.href='{{ route('jda_detail',['id' => $r->id]) }}';
                }
            },
