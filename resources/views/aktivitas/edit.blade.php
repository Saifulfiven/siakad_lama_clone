        ...
        <div class="col-md-12">
          <section class="panel">
            ...
            <div class="panel-body" style="padding-top: 3px">
                ...
                <form action="{{ route('akm_update') }}" id="form-aktivitas" class="form-horizontal" method="post" data-collabel="3" data-alignlabel="left">
                    {{ csrf_field() }}
                            ...
                            <div class="form-group">
                                ...
                                <div style="position: relative;">
                                    <div class="input-icon right"> 
                                        ...
                                        <input type="text" class="form-control" required="" id="autocomplete-mhs" name="nama_mhs" value="{{ $mhs->nim.' - '.$mhs->nm_mhs }}">
                                ...
                                <div>
                                    <select name="status" class="form-control mw-2">
                                        ...
                                        @foreach( Sia::statusAkmMhs() as $ak )
                                            <option value="{{ $ak->id_stat_mhs }}" {{ $mhs->status_mhs == $ak->id_stat_mhs ? 'selected':''}}>{{ $ak->nm_stat_mhs }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <?= Sia::TextfieldEdit('IPS','ips',str_replace('.',',',number_format($mhs->ips,2)),false,'text','','mw-1','onkeypress="return indeksPrestasi(event,this)" minlength="4"') ?>
                            <?= Sia::TextfieldEdit('IPK','ipk',str_replace('.',',',number_format($mhs->ipk,2)),false,'text','','mw-1','onkeypress="return indeksPrestasi(event,this)" minlength="4"') ?>
                            <?= Sia::TextfieldEdit('Jumlah sks semester','sks_semester',$mhs->sks_smt,false,'number','','mw-1') ?>
                            <?= Sia::TextfieldEdit('Jumlah sks total','sks_total',$mhs->sks_total,false,'number','','mw-1') ?>
