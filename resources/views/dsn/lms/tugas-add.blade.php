...
@section('content')
    ...
    <?php
        $tgl_mulai = Carbon::now()->format('d-m-Y H:i');
        $tgl_berakhir = '';
        if ( !empty($jadwal_ujian) ) {
            $tgl_mulai = Carbon::parse($jadwal_ujian->tgl_ujian.' '.$jadwal_ujian->jam_masuk)->format('d-m-Y H:i');
            $tgl_berakhir = Carbon::parse($jadwal_ujian->tgl_ujian.' '.$jadwal_ujian->jam_selesai)->format('d-m-Y H:i');
        }
    ?>
        ...
        <div class="col-md-12">
          <section class="panel" style="padding-bottom: 50px">
            ...
            <div class="panel-body" style="padding-top: 13px">
                ...
                <form action="{{ route('dsn_lms_tugas_store') }}" id="form-add" method="post" data-collabel="3" data-alignlabel="left">
                    {{ csrf_field() }}
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="col-md-4" style="padding-left: 0">
                                <div class="form-group">
                                    ...
                                    <select name="jenis" class="form-control" onchange="jenisTugas(this.value)">
...
<script>
    ...
    $(function () {
        ...
        $('#fallback').on('hidden.bs.modal', function(){
            $('#tab1').trigger('click');
        });
    ...
    function jenisTugas(value)
    {
        if ( value == 'ujian' ) {
            $('input[name="mulai_berlaku"]').val('{{ $tgl_mulai }}');
            $('input[name="tgl_berakhir"]').val('{{ $tgl_berakhir }}');
            $('input[name="tgl_tutup"]').val('{{ $tgl_berakhir }}');
        } else {
            $('input[name="mulai_berlaku"]').val("{{ Carbon::now()->format('d-m-Y H:i') }}");
            $('input[name="tgl_berakhir"]').val('');
            $('input[name="tgl_tutup"]').val('');
        }
    }
