...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="active">Kuis Detail</li>
    </ol>
            ...
            <section class="panel">
                <header class="panel-heading">
                    ...
                    <div class="pull-right">
                        <div class="btn-group">
                            <button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown"> <i class="fa fa-plus"></i> Tambah Soal <span class="caret"></span> </button>
                            <ul class="dropdown-menu custom-dropdown align-xs-left" role="menu" style="background-color: #0075b0">
                                <li><a href="{{ route('kuis_add_soal', ['id' => $r->id, 'id_kuis' => $kuis->id]) }}"><i class="fa fa-plus"></i> Buat Baru</a></li>
                                <li><a href="javascript:void(0);" data-toggle="modal" data-target="#modal-bank-soal">Dari Bank Soal</a></li>
                            </ul>
                        </div>

                        
                        <a href="{{ route('kuis_edit', ['id' => $r->id, 'id_kuis' => $kuis->id]) }}" class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i> Ubah Kuis</a>
                        <!-- <a href="{{ route('kuis_delete', ['id' => $r->id]) }}" class="btn btn-danger btn-sm" onclick="return confirm('Anda ingin menghapus kuis ini?')" title="hapus"><i class="fa fa-times"></i></a> -->
                        <a href="{{ route('dsn_lms', ['id' => $r->id]) }}" class="btn btn-default btn-sm">Kembali</a>
                    </div>
                </header>
                <div class="panel-body" style="padding-top: 13px">
                    ...
                    <div class="row">
                        <div class="col-md-6" style="margin-top: 20px;">
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tr>
                                        <td>Jumlah Soal</td>
                                        <td>: {{ Sia::jumlahSoal($kuis->id) }} &nbsp; 
                                            <button class="btn btn-primary btn-xs" data-toggle="modal" data-target="#modal-soal">Kelola Soal</button>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td>Jumlah yang telah mengerjakan</td>
                                        <td>: {{ Sia::jumlahMengerjakanKuis($kuis->id) }}</td>
                                    </tr>
                                    <tr>
                                        <td>Tanggal Mulai</td>
                                        <td>: {{ !empty($kuis->tgl_mulai) ? Rmt::tgl_indo($kuis->tgl_mulai).' '.substr($kuis->tgl_mulai, 11).' WITA' : 'Tidak terbatas' }} </td>
                                    </tr>
                                    <tr>
                                        <td>Tanggal Berakhir</td>
                                        <td>: {{ !empty($kuis->tgl_tutup) ? Rmt::tgl_indo($kuis->tgl_tutup).' '.substr($kuis->tgl_tutup, 11).' WITA' : 'Tidak terbatas' }} </td>
                                    </tr>
                        ...
                        <div class="col-md-12">
                            <hr>
                            <a href="{{ route('kuis_jawaban', ['id_jadwal' => $r->id, 'id' => $kuis->id]) }}?act=grade" class="btn btn-primary"><i class="fa fa-trophy"></i> Lihat Hasil Kuis</a>
                        </div>
                    </div>
                </div>
            </section>
                        ...
                        @if ( $so->jenis_soal == 'pg' )
                            <section class="panel">
                                <header class="panel-heading">
                                    ...
                                    <div class="pull-right">
                                        <a href="{{ route('kuis_edit_soal', ['id' => $r->id, 'id_kuis' => $kuis->id, 'id_soal' => $so->id]) }}?id_kuis_soal={{ $so->id_kuis_soal }}" class="btn btn-warning btn-xs">Ubah</a> &nbsp; 
                                        <a onclick="return confirm('Anda ingin menghapus soal ini?')" href="{{ route('kuis_delete_soal', ['id_kuis_soal' => $so->id_kuis_soal, 'id_jadwal' => $r->id]) }}" class="btn btn-danger btn-xs">Hapus</a>
                                    </div>
                                </header>
                                <div class="panel-body" style="padding-top: 13px">
                                    {!! $so->soal !!}
