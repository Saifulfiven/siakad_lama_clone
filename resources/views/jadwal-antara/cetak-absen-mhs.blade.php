...
@foreach( $dosen as $val )
	...
	<center>
	<h4>ABSENSI MAHASISWA SEMESTER PENDEK TAHUN {{ Sia::sessionPeriode('nama') }}</h4>
	</center>
			...
			@if ( $r->id_prodi == '61101' )
				...
				<tr>
	                <?php for ( $per = $start; $per <= $jml_pertemuan; $per++ ) { ?>
	                    <th style="padding:1px;font-size:11px" class="text-center" width="30">
	                        <?= $per ?>
	                    </th>
	                <?php } ?>
				</tr>
			...
			@foreach( $mahasiswa as $res )
				<tr>
					...
					@if ( $r->id_prodi == '61101' )
						...
						@endfor
					@else
						@for( $kol = 1; $kol <= $jml_pertemuan; $kol++ )
							<td></td>
						@endfor
					@endif
				</tr>
