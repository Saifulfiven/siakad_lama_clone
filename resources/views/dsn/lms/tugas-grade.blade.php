...
@section('content')
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('dsn_lms_tugas_detail', ['id_jadwal' => $r->id, 'id' => $tugas->id]) }}">Detail Tugas</a></li>
        ...
        <div class="col-md-12">
          ...
          <section class="panel">
            <header class="panel-heading">
                {{ $tugas->judul }}
                <button class="btn btn-default btn-xs pull-right" onclick="window.history.back()">Kembali</button>
            </header>
            <div class="panel-body" style="padding-top: 13px">
                ...
                @if ( !empty($tugas->file) )
                    ...
                    <a href="{{ route('dsn_lms_tugas_view_att', ['id_tugas' => $tugas->id, 'id_dosen' => Sia::sessionDsn(), 'file' => $tugas->file]) }}" download="">{{ $tugas->file }}</a>
                @endif
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="pull-left">
                            ...
                            @else
                                <a href="{{ route('dsn_lms_tugas_download', ['id_jadwal' => $r->id, 'id_tugas' => $tugas->id, 'judul' => $r->nm_mk.' - '.$tugas->judul]) }}" class="btn btn-primary btn-sm"><i class="fa fa-download"></i>Download Semua File</a>
                            @endif
                        </div>
                                        ...
                                        <tr<?= $status ? ' style="background: #f2dede"':'' ?>>
                                            ...
                                            <td>
                                                @if ( !empty($icon) )
                                                    ...
                                                    <a href="{{ route('dsn_lms_tugas_download_single', $tugas->id) }}?id_jadwal={{ $r->id }}&nim={{ $ps->nim }}&nama={{ $ps->nm_mhs }}&file={{ $jawab->file }}">{{ str_limit($jawab->file,20) }}</a>
                                                @endif
                                            </td>
                                            <td align="center">
