...
@section('content')
    ...
    <div id="content">
      <div class="row">
        <div class="col-md-12" style="padding-bottom: 30px">
                ...
                <div class="panel-footer">
                    ...
                    <div class="text-gray-600">
                        <a class="text-green-darker mr-2">
                            <?php if ( $topik->creator == Sia::sessionDsn() ) { ?>
                                Anda
                            <?php } else {
                            ...
                            <div class="text-gray-600">
                                ...
                                @if ( $val->id_user == Sia::sessionDsn() )
                                    <div class="pull-right">
                                        ...
                                        <a href="<?= route('dsn_lms_topik_reply_toggle_delete', ['id' => $val->id, 'id_topik' => $topik->id, 'deleted' => $val->is_deleted == 1 ? 0:1, 'id_dosen' => $topik->id_dosen]) ?>" onclick="return confirm('Anda ingin <?= $val->is_deleted == 1 ? 'mengembalikan':'menghapus' ?> data ini.?')" class='btn <?= $class_btn_toggle ?> btn-xs'><?= $btn_toggle ?></a>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            @else
                <form action="{{ route('dsn_lms_topik_reply', ['id' => $topik->id]) }}" method="post" id="form-reply">
                    {{ csrf_field() }}
                ...
                <hr>
                <center>
                    <form action="{{ route('dsn_lms_topik_tutup', ['id' => $topik->id, 'id_dosen' => $topik->id_dosen]) }}" id="form-tutup-diskusi">
                        {{ csrf_field() }}
    ...
    <div id="modal-edit-reply" class="modal fade" data-width="700" tabindex="-1" style="top: 30%">
        ...
        <div class="modal-body" style="padding-top: 0;">
            <div class="row">
                <div class="col-lg-12">
                    <form action="{{ route('dsn_lms_topik_reply_update') }}" method="post" id="form-edit-kiriman">
