...
@section('content')
    ...
    <div id="content" style="padding: 10px">
      <div class="row">
        <div class="col-md-12" style="padding-bottom: 30px">
            <h5 class="font-bold" style="margin-bottom: 10px">{{ $topik->judul }}
                <!-- <a class="btn btn-success btn-xs btn-loading pull-right" href="{{ route('dsnm_lms', ['id_jdk' => $r->id, 'id_dosen' => Request::get('id_dosen')]) }}">Kembali</a> -->
                <a class="btn btn-success btn-xs btn-loading pull-right" href="javascript:;" onclick="goBack()">Kembali</a>
                            ...
                            <div class="text-gray-600">
                                ...
                                @if ( $val->id_user == $id_dosen )
                                    <div class="pull-right">
                                        ...
                                        <a href="<?= route('dsnm_lms_topik_reply_toggle_delete', ['id' => $val->id, 'id_topik' => $topik->id, 'deleted' => $val->is_deleted == 1 ? 0:1, 'id_dosen' => $topik->id_dosen]) ?>" onclick="return confirm('Anda ingin <?= $val->is_deleted == 1 ? 'mengembalikan':'menghapus' ?> data ini.?')" class='btn <?= $class_btn_toggle ?> btn-xs'><?= $btn_toggle ?></a>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            @else
                <form action="{{ route('dsnm_lms_topik_reply', ['id' => $topik->id]) }}" method="post" id="form-reply">
                    {{ csrf_field() }}
                ...
                <hr>
                <center>
                    <form action="{{ route('dsnm_lms_topik_tutup', ['id' => $topik->id, 'id_dosen' => $topik->id_dosen]) }}" id="form-tutup-diskusi">
                        {{ csrf_field() }}
    ...
    <div id="modal-edit-reply" class="modal fade" data-width="700" tabindex="-1" style="top: 30%">
        ...
        <div class="modal-body" style="padding-top: 0;">
            <div class="row">
                <div class="col-lg-12">
                    <form action="{{ route('dsnm_lms_topik_reply_update') }}" method="post" id="form-edit-kiriman">
                        <input type="hidden" name="id" id="id">
                        {{ csrf_field() }}
                        <div class="form-group">
                            <label>Konten</label>
                            <textarea name="konten" id="konten" rows="9" class="form-control"></textarea>
                        </div>
        ...
        var options = {
            ...
            success:function(data, status, message) {
                ...
                if ( data.length === 0 ) {
                    ...
                    $('.reply-'+id).find('a.edit').removeAttr('data-konten');
