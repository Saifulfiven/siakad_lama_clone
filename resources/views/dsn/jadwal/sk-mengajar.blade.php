...
$pendek = !empty(Request::get('jenis')) && Request::get('jenis') == 2 ? 'Pendek' : '';
?>
...
<div class="print">
	...
	<?php 
		...
		$jenis = empty(Session::get('jdm.jenis')) ? 1 : Session::get('jdm.jenis');
	...
	<p>Pertama : Bahwa Saudara/i : <b> <?= Sia::namaDosen($dsn->gelar_depan,$dsn->nm_dosen,$dsn->gelar_belakang) ?></b> ditugaskan sebagai Dosen Pengasuh Mata Kuliah Pada {{ config('app.itb') }} {{ config('app.ni') }}
		untuk Tahun Akademik <b><?= Sia::nmSmt(Session::get('jdm.ta'), $jenis); ?> <?= $pendek ?> </b>sebagai berikut:
	...
	<table border="0">
		...
		<tr>
			...
			<td>Pada Tanggal</td>
			<td> : </td>
			<td><?= Rmt::tgl_indo(Request::get('tgl')) ?></td>
		</tr>
		<tr>
			<td></td>
